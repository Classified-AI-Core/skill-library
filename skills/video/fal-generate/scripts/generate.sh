...
# Backward compatibility: if legacy IMAGE_URL is set but array is empty, seed array
if [ -n "$IMAGE_URL" ] && [ ${#IMAGE_URLS[@]} -eq 0 ]; then
    IMAGE_URLS+=("$IMAGE_URL")
fi

# Build the request payload safely (escape prompt/URLs correctly)
if [[ "$MODEL" == *"image-to-video"* ]] || [[ "$MODEL" == *"i2v"* ]]; then
    if [ ${#IMAGE_URLS[@]} -eq 0 ] && [ -z "$IMAGE_URL" ]; then
        echo "Error: --image-url is required for image-to-video models" >&2
        exit 1
    fi
    PRIMARY_IMAGE_URL="${IMAGE_URLS[0]:-$IMAGE_URL}"
    PAYLOAD=$(python3.12 -c 'import json,sys; print(json.dumps({"prompt": sys.argv[1], "image_url": sys.argv[2]}))' "$PROMPT" "$PRIMARY_IMAGE_URL")
elif [[ "$MODEL" == *"video"* ]] || [[ "$MODEL" == *"veo"* ]] || [[ "$MODEL" == *"text-to-video"* ]]; then
    PAYLOAD=$(python3.12 -c 'import json,sys; print(json.dumps({"prompt": sys.argv[1]}))' "$PROMPT")
elif [[ "$MODEL" == *"/edit"* ]]; then
    # Image-conditioned edit endpoints require image_urls (array), even for a single image.
    if [ ${#IMAGE_URLS[@]} -eq 0 ] && [ -n "$IMAGE_URL" ]; then
        IMAGE_URLS+=("$IMAGE_URL")
    fi
    if [ ${#IMAGE_URLS[@]} -eq 0 ]; then
        echo "Error: --image-url is required for edit models" >&2
        exit 1
    fi
    PAYLOAD=$(python3.12 -c 'import json,sys; print(json.dumps({"prompt": sys.argv[1], "image_urls": sys.argv[3:], "num_images": int(sys.argv[2])}))' "$PROMPT" "$NUM_IMAGES" "${IMAGE_URLS[@]}")
else
    if [ ${#IMAGE_URLS[@]} -gt 1 ]; then
        PAYLOAD=$(python3.12 -c 'import json,sys; print(json.dumps({"prompt": sys.argv[1], "image_urls": sys.argv[4:], "image_size": sys.argv[2], "num_images": int(sys.argv[3])}))' "$PROMPT" "$IMAGE_SIZE" "$NUM_IMAGES" "${IMAGE_URLS[@]}")
    elif [ ${#IMAGE_URLS[@]} -eq 1 ]; then
        PAYLOAD=$(python3.12 -c 'import json,sys; print(json.dumps({"prompt": sys.argv[1], "image_url": sys.argv[2], "image_size": sys.argv[3], "num_images": int(sys.argv[4])}))' "$PROMPT" "${IMAGE_URLS[0]}" "$IMAGE_SIZE" "$NUM_IMAGES")
    elif [ -n "$IMAGE_URL" ]; then
        PAYLOAD=$(python3.12 -c 'import json,sys; print(json.dumps({"prompt": sys.argv[1], "image_url": sys.argv[2], "image_size": sys.argv[3], "num_images": int(sys.argv[4])}))' "$PROMPT" "$IMAGE_URL" "$IMAGE_SIZE" "$NUM_IMAGES")
    else
        PAYLOAD=$(python3.12 -c 'import json,sys; print(json.dumps({"prompt": sys.argv[1], "image_size": sys.argv[2], "num_images": int(sys.argv[3])}))' "$PROMPT" "$IMAGE_SIZE" "$NUM_IMAGES")
    fi
fi
...
# Check for error in result (match top-level "error" key, not substring in description text)
if echo "$RESULT" | python3.12 -c "import sys,json; d=json.load(sys.stdin); sys.exit(0 if 'error' in d else 1)" 2>/dev/null; then
    ERROR_MSG=$(echo "$RESULT" | python3.12 -c "import sys,json; d=json.load(sys.stdin); e=d.get('error',{}); print(e.get('message','') if isinstance(e,dict) else str(e))" 2>/dev/null)
    echo "Error: $ERROR_MSG" >&2
    exit 1
fi
