---
name: memelord
description: AI-powered meme generation, meme editing, and meme video generation using the Memelord API. Use when the
...
3. (Optional) create an `outputs/` folder to keep downloaded memes tidy:
   ```bash
   mkdir -p /home/workspace/Skills/memelord/outputs
   ```
